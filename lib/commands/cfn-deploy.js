...
export default async (conf) => {
  ...
  const parameters = conf.STACK_PARAMETERS;
  const templateURL = [
  ...
  if (!existing) {
    ...
    log.debug('Update parameters:', JSON.stringify(params));
    try {
      await cloudformation.updateStack(params).promise();
    } catch (e) {
      // TODO this better, using changeset
...
async function logStackEvents(stackName, waitForEvent) {
  ...
  let lastEventId;
  log.trace(`Waiting for CF event ${waitForEvent}`);
  // TODO this but better. need to detect faluire scenarios and non-zero exit
  cloudformation.waitFor(waitForEvent, { StackName: stackName }).promise().then(() => {
    log.trace('CF wait for complete');
    keepLogging = false;
  }).catch((e) => {
    log.error(e);
    keepLogging = false;
